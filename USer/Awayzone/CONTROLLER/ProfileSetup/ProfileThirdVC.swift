...
class ProfileThirdVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func userSignup() -> Void {
        ...
        let paramDict = ["culture"      :self.convertToJsonString(arr: objProfiledModel.culture),
                         ...
                         "first_name"    :objProfiledModel.first_name,
                         "user_culture"  :objProfiledModel.user_culture,
                         "culture_id"    :objProfiledModel.culture_id
        ]
        
        print(paramDict)
        
        ActivityLoadViewClassObj.showOverlay(self.view)
        WebserviceClass.sharedInstance.apiCallWithParametersUsingPOST(methodName: KFullRegistration, paramDictionary: paramDict as NSDictionary?, onCompletion: { (responseDict) in
